// ==UserScript==
// @name         🧲 V11_MAGNETAR — Liquidity Magnet | Micro-Tick Precision
// @namespace    candle-pro-strategy-v11-magnetar
// @version      11.0.0
// @description  V11: Sequential Handshake | Nano-UID | Wilder RSI | Temporal Divergence | Full-Reset | Protected Pulse
// @author       aoirusra
...
(function (W) {
  'use strict';
  if (W.__CANDLE_BOT_V11_MAGNETAR) return;
  W.__CANDLE_BOT_V11_MAGNETAR = true;
  ...
  const CFG = {
    ...
    SIGNAL_WATCHER_MS          : 200,  // ✅ v10.5: Signal Watcher interval
    SIGNAL_WATCHER_EXPIRY_MS   : 4000, // ✅ v10.5: الإشارة تنتهي بعد 4 ثوانٍ
    ...
    // ─── 🔥 [IMDB] Instant Multi-Double — حسب ثقة التوافق ──────────
    // الحد الأقصى للتوافق = 8.5 نقطة
    // 70% من 8.5 ≈ 6.0 → صفقتان فوريتان
    // 90% من 8.5 ≈ 7.6 → ثلاث صفقات فورية
    // 100% من 8.5 = 8.0+ → أربع صفقات فورية
    IMDB_ENABLED           : true,
    IMDB_CONF_MAX          : 8.5,    // أعلى نقطة توافق ممكنة
    IMDB_TIER_DOUBLE       : 6.0,    // ≥70% → 2 صفقات في نفس الثانية
    IMDB_TIER_TRIPLE       : 7.2,    // ≥85% → 3 صفقات في نفس الثانية
    IMDB_TIER_QUAD         : 8.0,    // ≥94% → 4 صفقات في نفس الثانية
  ...
  function shouldDouble(confluenceScore, signalDir, tveResult, macdResult) {
    ...
    // حرس 1: عتبة التوافق
    if (confluenceScore < CFG.DOUBLE_MIN_CONFLUENCE)         return { doDouble: false };
  ...
  function getIMDBConfPct(confScore) {
    return Math.round((confScore / CFG.IMDB_CONF_MAX) * 100);
  }
  ...
  function executeIMDB(signal, asset, confScore) {
    ...
    addLog(
      ...
      ' | ثقة: ' + pct + '% (' + confScore.toFixed(1) + '/8.5)',
      'signal'
    );
  ...
  function recordTrade(win, isTVE) {
    ...
    // Micro-adjust APEX-PRED adaptive weights based on last trade outcome
    (function _apexLearn(won) {
      const snap = _PS.snap;
      if (!snap) return;
      const lr = 0.02; // learning rate — small to avoid oscillation
      const aw = _PS.aw;
      // Reward indicators that agreed with the winning direction, penalize otherwise
      const sign = won ? 1 : -1;
      if (Math.abs(snap.vel)    > 0.3) aw.vel      = Math.max(0.5, Math.min(5.0, aw.vel      + sign*lr));
      if (Math.abs(snap.accel)  > 0.3) aw.accel    = Math.max(0.5, Math.min(5.0, aw.accel    + sign*lr));
      if (Math.abs(snap.tve)    > 0.3) aw.tveAccel = Math.max(0.5, Math.min(5.0, aw.tveAccel + sign*lr));
      if (Math.abs(snap.ofi)    > 0.3) aw.ofi      = Math.max(0.5, Math.min(5.0, aw.ofi      + sign*lr));
      if (Math.abs(snap.lr)     > 0.3) aw.lr       = Math.max(0.5, Math.min(5.0, aw.lr       + sign*lr));
      if (Math.abs(snap.roc)    > 0.3) aw.roc      = Math.max(0.5, Math.min(5.0, aw.roc      + sign*lr));
      if (Math.abs(snap.candle) > 0.3) aw.candle   = Math.max(0.5, Math.min(5.0, aw.candle   + sign*lr));
    })(win);
  ...
  function _onTVESignal(result, asset) {
    ...
    // ✅ v10.10 [IMDB]: TVE مع توافق عالٍ → صفقات متعددة فورية
    const tveConf = conf ? conf.score : 0;
    if (fastCloseAt && fastCloseAt > now + 100) {
      schedulePredictiveEntry();
    } else if (tveConf > 0 && getIMDBTier(tveConf) > 1 && canIMDB(tveConf)) {
      executeIMDB(result.signal, asset, tveConf);
  ...
  function schedulePredictiveEntry() {
    ...
    // ✅ v10.10.2 [SEE]: احسب وقت الدخول المبكر بذكاء حسب قوة الإشارة
    const confScore = _readySignal?.confluence?.score ?? 0;
    const earlyMs   = getSmartEarlyMs(confScore);
    const fireAt    = fastCloseAt - earlyMs;
    const now       = Date.now();
    if (fireAt <= now + 30) return;
    addLog('[SEE] 📅 دخول مبكر: '+(earlyMs/1000).toFixed(2)+'ث قبل الإغلاق | CONF:'+confScore.toFixed(1), 'info');
  ...
  function onWindowEntry(asset, period) {
    ...
    if (result) {
      ...
      // ⚡ v10.2: عتبات تكيفية حسب الفريم
      const thresholds = getAdaptiveThresholds(candlePeriod);
      const reqScore  = autoTrade ? thresholds.auto : thresholds.min;
      const confScore = result.confluence?.score ?? 0;

      if (confScore < reqScore) {
        addLog('⏸ انتظار توافق أعلى ('+confScore.toFixed(1)+'/'+reqScore+' للفريم '+candlePeriod+'ث)','info');
      } else if (autoTrade) {
        // ✅ v10.10 [IMDB]: حدّد عدد الصفقات حسب نسبة الثقة
        const imdbTier = getIMDBTier(confScore);
        if (imdbTier > 1 && canIMDB(confScore)) {
          executeIMDB(result.signal, asset, confScore);
        } else {
          // فحص الصفقة المزدوجة القديم (احتياطي)
          const macdData = result.confluence?.macdData || null;
          const dbl = shouldDouble(confScore, result.signal, null, macdData ? { signal: getMACDSignal(candles) } : null);
          if (dbl.doDouble) {
  ...
  // Only execute if the 22-algorithm engine agrees with the intended direction
  // AND has enough confidence. This is the highest-precision gate.
  {
    const _apexDir  = _PS.direction;
    const _apexConf = _PS.confidence;
    const _apexAgree = _PS.snap?.agree ?? 0;
    const _apexTotal = _PS.snap?.total ?? 23;
    const _agreePct  = _apexTotal > 0 ? _apexAgree / _apexTotal : 0;

    // Require direction match AND confidence > 0.30 AND at least 40% indicator agreement
    const _apexOk = (_apexDir === direction || _apexDir === 'NEUTRAL')
                    && _apexConf >= 0.30
                    && _agreePct >= 0.40;

    if (!_apexOk) {
      addLog(
        '🔬 [H5] APEX-PRED rejected ' + direction +
        ' — engine says ' + _apexDir +
        ' (conf:' + (_apexConf*100).toFixed(0) + '% | agree:' + _apexAgree + '/' + _apexTotal + ')',
        'error',
        'regime:' + (_PS.snap?.regime ?? '?') + ' hurst:' + (_PS.hurst_h?.toFixed(2) ?? '?')
      );
      _readySignal = null;  // V11 FIX: always clear — prevents Watcher from re-firing already-rejected signal
      return;
    }
  }
  // ════════════════════════════════════════════════════════════════════════
    ...
    addLog(
      ...
      ' | فريم:' + (candlePeriod||'؟') + 'ث' +
      ' | APEX:' + _PS.direction +
      ' conf:' + Math.round(_PS.confidence*100) + '%' +
      ' agree:' + (_PS.snap?.agree??0) + '/23' +
      ' regime:' + (_PS.snap?.regime??'?') +
      ' hurst:' + (_PS.hurst_h?.toFixed(2)??'?')
    );
  ...
  // ══════════════════════════════════════════════════════════════════════
  // ✅ v10.5 [WATCH] Signal Watcher — طبقة أمان ثانية للتنفيذ
  // ══════════════════════════════════════════════════════════════════════
  function _startSignalWatcher() {
    _v11_setInterval(() => {
      ...
      // فحص الكولداون التكيفي
      if ((now - lastTradeMs) < getAdaptiveCooldown()) return;
      if (tradeExec) return;
      if (_lossStreakPauseUntil > now) return;
      // ✅ v10.9: انتظر انتهاء الصفقة المعلقة (نافذة النتيجة)
      // على الفريمات القصيرة (≤15ث) لا نُطبق هذا الشرط لأن الصفقات تتداخل
      ...
      // ✅ v10.10 FIX#2: PAT أيضاً مُحجوب في الضعيف — نفس منطق executeTrade H4
      ...
      if (_wCandles.length >= 3) {
        ...
        // ✅ v10.10 FIX#2: حجب الكل (TVE+PAT) في UP_WEAK/DN_WEAK
      ...
      // APEX-PRED real-time direction guard inside Signal Watcher
      const _apexCheck = _PS.direction;
      if (_apexCheck !== 'NEUTRAL' && _apexCheck !== dir) {
        addLog('🔬 [SW-APEX] Signal cancelled — APEX-PRED:' + _apexCheck
               + ' vs signal:' + dir, 'info');
        _readySignal = null;
        return;
      }

      addLog('[WATCH⚡] تنفيذ من Signal Watcher: '+dir,'signal');
      _readySignal = null;  // ✅ v10.10.2: امسح قبل التنفيذ — يمنع الإطلاق المزدوج مع IMM
      executeTrade(dir, a);
  ...
  function _updateConfDisplay(conf) {
    ...
    if (dblEl) {
      const tier = getIMDBTier(conf.score);
      const pct  = getIMDBConfPct(conf.score);
      if (tier >= 4) {
      ...
      } else if (conf.score >= CFG.DOUBLE_MIN_CONFLUENCE) {
        dblEl.textContent='🔥 DBL'; dblEl.className='cb-ind-badge active';
      } else {
        dblEl.textContent='DBL–'; dblEl.className='cb-ind-badge';
      }
    }
  }
  ...
  // ══════════════════════════════════════════════════════════════════════
  // § 31  🔴 [APEX-PRED] محرك التنبؤ الفوري المتقدم — الإصدار المتطور
  //
  //  ✦ 22 خوارزمية تعمل بالتوازي على كل تيك
  //  ✦ أنظمة: OFI · LR · TickRSI · MarketRegime · MomentumDecay
  //           HurstApprox · ZScore · VelocityMesh · EMA-Stack(8)
  //           AutoCorr · Entropy · PriceGeometry · Divergence-Tick
  //           ROC-Multi · VolAdj · TrendConsensus · PatternProb
  //           OrderBlockDetect · BreakoutProb · FractalPulse
  //           SmartSmoothing · AdaptiveWeight
  //  ✦ يتنبأ قبل الحدث — يكشف الانعكاس قبل بدايته
  // ══════════════════════════════════════════════════════════════════════
  ...
  const _PS = {
    ...
    ac_prev_vel: null, ac_sum_xy: 0, ac_sum_x: 0, ac_sum_y: 0,
    ac_sum_x2: 0, ac_n: 0,
    ...
    hurst_rs4: [], hurst_rs8: [], hurst_rs16: [], hurst_h: 0.5,
    ...
    aw: {
      vel:2.5, accel:2.2, emaStack:2.0, tveAccel:2.5, tveBias:1.0,
      tRsi:1.8, ofi:2.0, lr:1.8, mom:1.5, vol:0.8,
      candle:1.5, rsi:1.2, emaC:1.0, macd:0.9, bb:0.8,
      srsi:0.8, mtf:0.6, sr:0.4, ac:1.2, entropy:0.7,
      hurst:0.6, geo:1.0, zScore:1.2, roc:1.3, breakout:1.5,
    },
    ...
    // ─ Prediction history (last 30 predictions) ───────────
    predHist: [],  // {dir, conf} — لحساب دقة التاريخية
  ...
  // ══════════ [C] الدالة الرئيسية — تُستدعى على كل تيك بأعلى أولوية ══
  function _predBarTick(asset, price, now) {
    ...
    // ════ H: Linear Regression slope ═════════════════════════════════
    let lrSlope = 0;
    {
      const lp = ps.lr_prices;
      const n  = lp.length;
      if (n >= 5) {
        let sx=0,sy=0,sxy=0,sx2=0;
        for (let i=0;i<n;i++){ sx+=i; sy+=lp[i]; sxy+=i*lp[i]; sx2+=i*i; }
        const denom = n*sx2-sx*sx;
        if (Math.abs(denom)>1e-15) lrSlope = (n*sxy-sx*sy)/denom;
      }
    }
    // نعيّر الـ slope بالتقلب
    const lrNorm = _cls(lrSlope / (ps.vol*10 || 1e-9), 1);

    // ════ I: AutoCorrelation (lag-1) على السرعات ═════════════════════
    // AutoCorr > 0 → زخم مستمر; AutoCorr < 0 → انعكاس محتمل
    let ac1 = 0;
    {
      const pv = ps.ac_prev_vel;
      if (pv !== null) {
        ps.ac_sum_xy += pv*vel; ps.ac_sum_x+=pv; ps.ac_sum_y+=vel;
        ps.ac_sum_x2 += pv*pv; ps.ac_n++;
        if (ps.ac_n > 30) { ps.ac_sum_xy-=pv*vel; ps.ac_sum_x-=pv; ps.ac_sum_y-=vel; ps.ac_sum_x2-=pv*pv; ps.ac_n--; }
        const n=ps.ac_n, denom=(n*ps.ac_sum_x2-(ps.ac_sum_x**2));
        if (Math.abs(denom)>1e-20) ac1 = (n*ps.ac_sum_xy-ps.ac_sum_x*ps.ac_sum_y)/denom;
      }
      ps.ac_prev_vel = vel;
    }
    ...
    // ════ L: Hurst Exponent (تقريب سريع) ════════════════════════════
    // H > 0.5 → اتجاه مستمر | H < 0.5 → انعكاس | H ≈ 0.5 → عشوائي
    {
      const _rs = (w) => {
        if (w.length < 4) return null;
        const m  = _mean(w);
        let cum=0, cumMax=-Infinity, cumMin=Infinity;
        let rng=0;
        for (const v of w) { cum+=v-m; if(cum>cumMax)cumMax=cum; if(cum<cumMin)cumMin=cum; }
        rng = cumMax - cumMin;
        const s = _std(w,m);
        return s>1e-12 ? rng/s : null;
      };
      const pv = ps.vels;
      if (pv.length >= 16) {
        const rs4  = _rs(pv.slice(-4));
        const rs8  = _rs(pv.slice(-8));
        const rs16 = _rs(pv.slice(-16));
        if (rs4!==null&&rs8!==null&&rs16!==null&&rs4>0&&rs8>0&&rs16>0) {
          // H ≈ log(R/S) / log(n)
          const H8  = Math.log(rs8)  / Math.log(8);
          const H16 = Math.log(rs16) / Math.log(16);
          ps.hurst_h = (H8+H16)/2;
        }
      }
    }

    // ════ M: EMA Stack Score — كم EMA أسفل من السعر؟ ════════════════
    let emaStackScore = 0;
    {
      ...
      if (total>0) emaStackScore = (above-below)/total; // -1..1
    }
    ...
    const _roc = (w) => {
      ...
      if (Math.abs(first)<1e-15) return 0;
      return (last-first)/Math.abs(first);
    };
    ps.roc3  = _roc(ps.W5.slice(-3));
    ps.roc8  = _roc(ps.W10);
    ps.roc15 = _roc(ps.W20.slice(-15));
    // اتفاق ROC — كلهم بنفس الاتجاه يعطي نقطة أعلى
    const rocAgree = [ps.roc3,ps.roc8,ps.roc15].map(r=>r>0?1:r<0?-1:0);
    const rocConsensus = rocAgree.reduce((a,v)=>a+v,0)/3; // -1..1

    // ════ O: Breakout Probability ═════════════════════════════════════
    let breakoutScore = 0;
    {
      const w = ps.W40;
      if (w.length >= 20) {
        ps.bp_n++;
        const max20 = Math.max(...w.slice(-20));
        const min20 = Math.min(...w.slice(-20));
        const range = max20 - min20 || 1e-9;
        const pos   = (price - min20) / range; // 0..1
        // تقاطع القمة → breakout صاعد محتمل
    ...
    // ════ P: Price Geometry — هل تتشكل قمة/قاع؟ ════════════════════
    let geoScore = 0;
    {
      const w = ps.W10;
      if (w.length >= 5) {
        const mid = Math.floor(w.length/2);
        const lh  = Math.max(...w.slice(0,mid));
        const rh  = Math.max(...w.slice(mid));
        const ll  = Math.min(...w.slice(0,mid));
        const rl  = Math.min(...w.slice(mid));
        // W-bottom محتمل: ll < lh && rl ≈ ll && price > rl
        if (ll < lh && Math.abs(rl-ll)<ps.vol*3 && price>rl) geoScore =  0.8;
        // M-top محتمل
        if (lh > ll && Math.abs(rh-lh)<ps.vol*3 && price<rh) geoScore = -0.8;
        // Higher-High
        if (rh > lh*1.0001) geoScore = Math.max(geoScore,  0.5);
        // Lower-Low
        if (rl  < ll*0.9999) geoScore = Math.min(geoScore, -0.5);
      }
    }

    // ════ Q: TVE Sigma (من محرك TVE الرئيسي) ════════════════════════
    let tveSigma = 0, tveBias = 0;
    {
      const vh = TickVelocityEngine.velHistory;
      if (vh.length >= 5) {
        const win  = vh.slice(-Math.min(25,vh.length));
        const vals = win.map(v=>v.accel);
        const m    = _mean(vals);
        const s    = _std(vals,m);
    ...
    // ════ R: مؤشرات الشموع ════════════════════════════════════════════
    const candles = candleBuffers[asset] || [];
    let candleTrend=0, rsiCandle=0, emaCross=0, macdS=0, bbS=0, srsiS=0, mtfS=0, srS=0;

    if (candles.length >= 3) {
      const tr = analyzeTrend(candles);
      candleTrend = tr.trend==='UP'?1:tr.trend==='DOWN'?-1:tr.trend==='UP_WEAK'?0.5:tr.trend==='DN_WEAK'?-0.5:0;
    }
    if (candles.length >= 9) {
      const r = computeRSIProxy(candles,8);
      if (r!==null) rsiCandle = r<30?1:r>70?-1:r<40?0.4:r>60?-0.4:0;
    }
    {
      const es = getEMACrossSignal(candles);
      emaCross = es==='BUY'?1:es==='SELL'?-1:0;
    }
    if (candles.length >= 35) {
      const m = computeMACD(candles);
      if (m) macdS = _cls(m.histogram*300,1) + (m.cross==='BUY'?0.3:m.cross==='SELL'?-0.3:0);
    }
    if (candles.length >= 20) {
      const b = computeBB(candles);
      if (b&&!b.squeeze) bbS = b.percentB<=0.1?1:b.percentB>=0.9?-1:b.percentB<=0.25?0.4:b.percentB>=0.75?-0.4:0;
    }
    if (candles.length >= 17) {
      const sr = computeStochRSI(candles);
      if (sr&&sr.signal) srsiS = sr.signal==='BUY'?Math.min(1,(20-Math.min(20,sr.k))/20):sr.signal==='SELL'?-Math.min(1,(Math.max(80,sr.k)-80)/20):0;
    }
    if (candles.length >= 6) {
      const htf = getHigherTFTrend(candles);
      if (htf) mtfS = htf.trend.startsWith('UP')?0.6:htf.trend.startsWith('DO')?-0.6:0;
      const srLvl = detectSRLevels(candles);
      srS = (srLvl.nearSupport?0.3:0) - (srLvl.nearResistance?0.3:0);
    }

    // ════ S: حساب التوافق بين المجموعات ═════════════════════════════
    // --- مجموعة 1: محركات التيك (أسرع) ---
    ...
    // --- Z-Score regime --- (في trend: فسّره كزخم؛ في ranging: عكسه)
    const isHurst  = ps.hurst_h > 0.6; // trending regime
    const zAdj     = isHurst ? _cls(zScore/2,1) : _cls(-zScore/2,1);

    // --- Autocorrelation amplifier ---
    // AutoCorr موجبة → تعزز السرعة الحالية؛ سالبة → تعكسها
    const acAmplified = _cls(velAvg * (1 + ac1*0.5), 1);

    // --- Entropy amplifier ---
    // clarity عالي → نضاعف الإشارة السائدة
    const entropyAmp = _cls(entropyScore, 1);

    const aw = ps.aw;

    // ════ T: الجمع الموزون النهائي ════════════════════════════════════
    // كل مؤشر يعطي نتيجة [-1..1] مضروبة في وزنه
    const components = [
      [velAvg,         aw.vel],
      [accelAvg,       aw.accel],
      [tveSigma,       aw.tveAccel],
      [tveBias,        aw.tveBias],
      [tRsiNorm,       aw.tRsi],
      [ofiNorm,        aw.ofi],
      [momNorm,        aw.mom],
      [lrNorm,         aw.lr],
      [emaStackScore,  aw.emaStack],
      [acAmplified,    aw.ac],
      [entropyAmp,     aw.entropy],
      [zAdj,           aw.zScore],
      [rocConsensus,   aw.roc],
      [breakoutScore,  aw.breakout],
      [geoScore,       aw.geo],
      [candleTrend,    aw.candle],
      [rsiCandle,      aw.rsi],
      [emaCross,       aw.emaC],
      [macdS,          aw.macd],
      [bbS,            aw.bb],
    ...
    const normScore = totalWeight > 0 ? rawScore / totalWeight : 0; // -1..1

    // ════ U: تحويل النتيجة إلى BUY% ═══════════════════════════════
    // نستخدم sigmoid لتحويل القيمة المعيارية إلى احتمالية
    const sigmoid = (x) => 1 / (1 + Math.exp(-x * 5));
    const rawBuyProb = sigmoid(normScore) * 100;

    // ════ V: نظام التنعيم الذكي (Adaptive Smoothing) ════════════════
    // كلما زادت سرعة التغيير (velocity عالية)، نزيد الـ smoothing factor
    const dynamicK = Math.min(0.45, Math.max(0.10, 0.18 + Math.abs(normScore)*0.15));
    ps.smoothK    = dynamicK;
    ...
    // ════ W: تحديد الاتجاه والثقة ════════════════════════════════════
    // الثقة تعتمد على: بُعد النسبة عن 50% + مستوى التوافق بين المجموعات
    const rawConf = Math.abs(buyPct - 50) / 50;
    // تعزيز الثقة عند اتفاق كثير من المؤشرات
    const agreeCount = components.filter(([v])=>Math.sign(v)===Math.sign(normScore)&&Math.abs(v)>0.1).length;
    const agreeFactor = agreeCount / components.length;
    const confidence  = Math.min(1, rawConf * (0.7 + agreeFactor*0.6));

    const direction = buyPct > 57 ? 'BUY' : sellPct > 57 ? 'SELL' : 'NEUTRAL';
    ...
    // ════ X: حفظ Snapshot للواجهة ════════════════════════════════════
    ps.snap = {
      vel:velAvg, accel:accelAvg, tve:tveSigma, ofi:ofiNorm, mom:momNorm,
      lr:lrNorm, ema:emaStackScore, ac:ac1, entropy:entropyScore, z:zAdj,
      roc:rocConsensus, brk:breakoutScore, geo:geoScore, hurst:ps.hurst_h,
      tRsi:ps.tRsi_val, regime:isHurst?'TREND':'RANGE', agree:agreeCount,
      total:components.length, candle:candleTrend, rsi:rsiCandle,
    };
  ...
  function _predBarRefreshUI() {
    ...
    // ── شريط الثقة (conf-fill) ─────────────────────────────────────────
    const confFillEl = W.document.getElementById('pb-conf-fill');
    if (confFillEl) confFillEl.style.width = (st.confidence * 100).toFixed(1) + '%';

    if (fillEl) {
      fillEl.style.width = st.buyPct + '%';
      const conf = st.confidence;
      if (st.direction === 'BUY') {
        const g = Math.round(180 + conf * 75);
        fillEl.style.background = `linear-gradient(90deg,#00d264,rgba(0,${g},100,0.85))`;
      } else if (st.direction === 'SELL') {
        fillEl.style.background = `linear-gradient(90deg,rgba(255,${Math.round(55-conf*40)},${Math.round(85-conf*60)},0.85),#ff3755)`;
      } else {
        fillEl.style.background = 'linear-gradient(90deg,#ffb020,#ff8c00)';
      }
    }

    // ── Direction badge (pb-direction-label) ───────────────────────────
    const dirLabelEl = W.document.getElementById('pb-direction-label');
    if (dirLabelEl) {
      const arrows = { BUY: '▲ BUY', SELL: '▼ SELL', NEUTRAL: '— NEUTRAL' };
      const colors = { BUY: '#00d264', SELL: '#ff3755', NEUTRAL: '#888' };
      dirLabelEl.textContent = arrows[st.direction] || '— NEUTRAL';
      dirLabelEl.style.color = colors[st.direction] || '#888';
    }
    if (dirEl) {
      const arrows = { BUY: '▲', SELL: '▼', NEUTRAL: '◆' };
      const colors = { BUY: '#00d264', SELL: '#ff3755', NEUTRAL: '#ffb020' };
    ...
    if (sigEl) {
      ...
      sigEl.textContent = 'ثقة: ' + pct + '%';
      sigEl.style.color = st.confidence > 0.6 ? '#00d264' : st.confidence > 0.3 ? '#ffb020' : 'rgba(255,255,255,0.4)';
    }

    // ── Market Regime (pb-regime) ─────────────────────────────────────
    const regimeEl = W.document.getElementById('pb-regime');
    if (regimeEl) {
      const regime = st.snap && st.snap.regime ? st.snap.regime : 'RANGE';
      regimeEl.textContent = regime === 'TREND' ? '📈 TREND' : '↔ RANGE';
      regimeEl.style.color = regime === 'TREND' ? '#00d264' : '#ffb020';
    } else {
      // Create regime element inside bar container
      const pbBar = W.document.getElementById('pb-bar');
      if (pbBar) {
        const rEl = W.document.createElement('span');
        rEl.id = 'pb-regime';
        rEl.style.cssText = 'font-size:9px;font-weight:700;letter-spacing:0.5px;margin-left:6px;';
        const regime = st.snap && st.snap.regime ? st.snap.regime : 'RANGE';
        rEl.textContent = regime === 'TREND' ? '📈 TREND' : '↔ RANGE';
        rEl.style.color = regime === 'TREND' ? '#00d264' : '#ffb020';
        pbBar.appendChild(rEl);
      }
    }

    // ── Algorithm Agreement (pb-agree) ────────────────────────────────
    const agreeEl = W.document.getElementById('pb-agree');
    const agreeCount = (st.snap && st.snap.agree !== undefined) ? st.snap.agree : 0;
    const agreeTotal = (st.snap && st.snap.total !== undefined) ? st.snap.total : 23;
    if (agreeEl) {
      agreeEl.textContent = '✓ ' + agreeCount + '/' + agreeTotal;
      const agreePct = agreeTotal > 0 ? agreeCount / agreeTotal : 0;
      agreeEl.style.color = agreePct >= 0.7 ? '#00d264' : agreePct >= 0.5 ? '#ffb020' : 'rgba(255,255,255,0.35)';
    } else {
      const pbBar = W.document.getElementById('pb-bar');
      if (pbBar) {
        const aEl = W.document.createElement('span');
        aEl.id = 'pb-agree';
        aEl.style.cssText = 'font-size:9px;font-weight:700;letter-spacing:0.5px;margin-left:6px;';
        const agreePct = agreeTotal > 0 ? agreeCount / agreeTotal : 0;
        aEl.textContent = '✓ ' + agreeCount + '/' + agreeTotal;
        aEl.style.color = agreePct >= 0.7 ? '#00d264' : agreePct >= 0.5 ? '#ffb020' : 'rgba(255,255,255,0.35)';
        pbBar.appendChild(aEl);
      }
    }

    // ── Hurst exponent display (pb-hurst) ─────────────────────────────
    const hurstEl = W.document.getElementById('pb-hurst');
    const hurstVal = (typeof st.hurst_h === 'number') ? st.hurst_h : 0.5;
    const hurstColor = hurstVal > 0.6 ? '#00d264' : hurstVal >= 0.45 ? '#ffb020' : '#ff3755';
    if (hurstEl) {
      hurstEl.textContent = 'H:' + hurstVal.toFixed(2);
      hurstEl.style.color = hurstColor;
    } else {
      const pbBar = W.document.getElementById('pb-bar');
      if (pbBar) {
        const hEl = W.document.createElement('span');
        hEl.id = 'pb-hurst';
        hEl.style.cssText = 'font-size:9px;font-weight:700;letter-spacing:0.5px;margin-left:6px;';
        hEl.textContent = 'H:' + hurstVal.toFixed(2);
  ...
  function _initPredictionBar() {
    ...
    root.innerHTML = `
      <div id="pb-bar">
        <div id="pb-header">
          <span id="pb-title">⚡ تنبؤ فوري V11</span>
  ...
  // ══════════════════════════════════════════════════════════════════════
  function init() {
    // V11: Strict Sequential Handshake — Signal Watcher stays dormant until UI is 100% verified
    const _startAfterUI = () => {
      initUI();
      _startSignalWatcher();
      addLog('⚡ V11_MAGNETAR — يعمل | IMDB فعّال | Signal Watcher ✓ | Wilder-RSI | Protected-Pulse', 'signal');
    };
    if (W.document.body) {
      _startAfterUI();
    } else {
